...
# load data
  ...
  2. **空白セルデータ**
    ...
    ここでの空白セルは、一部の大臣許可漁業の操業がないことを示している
...
# save
saveRDS(s1_dat_na_0.1, file = here::here("data","analyzed","s1_dat_na_0.1.rds"))
saveRDS(s2_dat_na_0.1, file = here::here("data","analyzed","s2_dat_na_0.1.rds"))
saveRDS(s1_dat_na_0.5, file = here::here("data","analyzed","s1_dat_na_0.5.rds"))
saveRDS(s2_dat_na_0.5, file = here::here("data","analyzed","s2_dat_na_0.5.rds"))
```
...
# S1/S2
s1_dat_na_0.1 <- readRDS(here::here("data","analyzed","s1_dat_na_0.1.rds"))
s2_dat_na_0.1 <- readRDS(here::here("data","analyzed","s2_dat_na_0.1.rds"))
s1_dat_na_0.5 <- readRDS(here::here("data","analyzed","s1_dat_na_0.5.rds"))
s2_dat_na_0.5 <- readRDS(here::here("data","analyzed","s2_dat_na_0.5.rds"))
...
## S1/S2(0.1度集計)
  ...
  GFWによるS1及びS2データのうち、0.1度単位で集計し地図上に可視化。AIS未搭載漁船の分布を捉える
...
## S1/S2(0.5度集計)
  ...
  GFWによるS1及びS2データのうち、0.5度単位で集計し地図上に可視化。解像度を粗くすることで、AIS未搭載漁船の分布の大まかな傾向を捉える。
...
## AIS
  ...
  GFWによるAISデータを0.1度単位で集計し地図上に可視化。AIS搭載漁船の分布の全体像を捉える。
...
```
  ...
  日本海側、特に北陸沖での空白セル同士の重なりが点在しており、AIS未搭載漁船の操業がないことを示している。また南西のEEZ境界線付近や九州・沖縄の西側の沖合にも空白セルの重なりが点在している。
...
　S2+空白セルと同様、北陸沖、南西のEEZ境界線付近、九州・沖縄の西側の沖合に空白セルの重なりが分布しており、AIS搭載漁船の操業が示されている。
...
```
  ...
  全データを統合した結果、日本海沖にいくつか空白セルの重なりが見られるものの、その数は非常に少ないことが確認。すなわち、ほとんどの海域ではAIS搭載・未搭載のいずれかの漁船が操業しており、完全に未利用と考えられる海域はほとんどないことが示唆された。
...
　そして3つのデータを統合した結果、操業が確認されない空白セルはごくわずかであり、大部分の海域ではAIS搭載・未搭載のいずれかの漁船が操業していることが明らかになった。すなわち、日本のEEZ内において、洋上風力発電の促進区域として指定できる海域はほとんどないことが示唆された。
　以上より、日本国内での洋上風力発電の導入において、漁業活動に全く支障を与えない海域を見出すことは極めて困難である。
